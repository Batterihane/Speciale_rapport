...
The graphs in figure \ref{completeIdenticalTreesGraph} shows the result of the experiment when the input trees are identical and when the leaves are randomly distributed in the trees. \todo{nonidentical trees}. From these graphs, we see that the runtime is slightly better when the input trees are similar. In both cases, it is easily seen that the runtime is worse than when inputting random trees, and again we see sudden increases in runtime when reaching certain tree sizes. Already at tree size \textasciitilde 42000, the JVM ran out of memory and could not continue the execution. 
...
In \todo{Graph with all experiment results - gc subtracted?} we see the runtime of each of the two algorithms \todo{include naive?}, given identical complete trees as input. Clearly, there is a huge improvement from the $O(n^2)$ algorithm to the $O(nlogn)$ algorithm and even for small input trees, the $O(nlogn)$ algorithm is dominant.
