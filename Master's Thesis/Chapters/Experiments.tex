...
The algorithm by Goddard et. al. \cite{nsquared} produces an $n*n$ table no matter the topology of the input trees The runtime of the algorithm must therefore be $O(n^2)$ for any input and we don't expect any significant difference in runtime when inputting trees of different topologies. \todo{verify?}.

In order to verify the runtime of $O(n^2)$, we ran the algorithm on randomly created trees and measured the runtime. The result is shown in \todo{figure}. \todo{move gc section here?}
