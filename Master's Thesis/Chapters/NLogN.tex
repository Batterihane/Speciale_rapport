...
Each node $v$ in a centroid path has a side tree, which is the subtree having as root the child of $v$ which is not on the centroid path. If $v$ is a leaf, the side tree is $v$ itself. The side tree of a node $u_i$ in $\pi$ is referred to as $M_i$ and has size $m_i$. The side tree of a node $v_j$ in a path $\pi(x)$ is referred to as $N_j$ and has size $n_j$. Figure ?? \todo{something like fig. 3.1 in article} illustrates the principle of centroid paths in $T_1$ and $T_2$.
...
In this section we will give an analysis of the amount of space that is used by the algorithm. We will walk through each step of the algorithm and deduce how much memory is used and finally conclude the total space consumption of the algorithm.

\subsection{Centroid decompositions}
\subsection{Inducing Subtrees}
\subsection{Recursive Calls ??}
\subsection{Creating Matching Graps}
\subsection{Computing LWAMs}
Saving a LWAM for each $u_i \in \pi$ for each matching graph. $O(n^2)$??
